...
close = data.at[0,'Prev Close']
count = tsum = 0

for index, row in data.iterrows():
	count += 1
	chg.append(round(row['Last'] - row['Prev Close'],2))
	pchg.append(round(((row['Last'] - row['Prev Close'])/row['Prev Close'])*100.0,2))
...
del_columns = ['Symbol', 'Series', 'Turnover']
data.drop(del_columns, inplace=True, axis=1) # Delete selected columns in dataframe
...
print("\n",data)
print("-"*197)
print("{0:<147} {1:<8.2f} {2:<8.2f} {3} {4:<8.2f}".format(' AVERAGE:',np.mean(chg),np.mean(pchg),'MA:',TMA))
print("-"*197)
