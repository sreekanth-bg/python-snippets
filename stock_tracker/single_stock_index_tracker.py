# Input days and stock name (only indices) as parameters. Ex: python3 single_stock_index_tracker.py 30 BANKNIFTY
...
data = pd.DataFrame(get_history(symbol=PARAM2, start=sday, end=eday, index=True))
...
close = data.at[0, 'Open']    # Access a single value using a label.
count = tsum = 0

for index, row in data.iterrows():
    count += 1
    chg.append(round(row['Close'] - row['Open'], 2))
    pchg.append(
        round(((row['Close'] - row['Open'])/row['Open'])*100.0, 2))
    tchg.append(round(row['Open'] - close, 2))
    ptchg.append(round(((row['Open'] - close)/close)*100.0, 2))
